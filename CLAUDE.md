...
- **Aprobaciones sin guard de idempotencia (duplicados en cascada):** Re-aprobar una etapa del flujo de ingreso (`aprobarPsico`, `aprobarPreocup`, `aprobarDocum`/`excepcionDocum`) creaba un registro nuevo en la etapa siguiente sin chequear si ya existía uno vivo, generando duplicados silenciosos en cascada. Lo destapó el candidato de prueba "Carlos" (DNI 35888777): psico atascado en `En proceso` + preocup duplicado, todo apareciendo como "En proceso" en los 3 módulos a la vez. Resuelto con guards de idempotencia en `aprobarPsico`, `aprobarPreocup` y `_crearAltaDesdeDocum` (este último devuelve booleano y los llamadores gatean el toast de éxito). Cada guard detecta un registro "vivo" por DNI y muestra un toast informativo en vez de duplicar.
...
- **Conciliación por DNI puede bloquear reingresos:** Los guards de idempotencia (y toda la conciliación entre etapas) matchean por DNI tratando los estados `En proceso`/`Aprobado` como "vivos". Si un asociado histórico (ya rechazado o dado de baja) vuelve a contratarse con el mismo DNI, el guard podría bloquear el re-procesamiento. No es problema activo hoy. Si en el futuro Gabi necesita re-procesar un DNI ya histórico, habrá que extender los guards para tratar `Rechazado`/`Anulado` como no-vigentes en esa lógica.

## Convenciones operativas de desarrollo

- **Heredoc + `grep -c` ante duplicación del preview:** El preview de Write/Edit a veces muestra líneas duplicadas o cortadas (artefacto visual; el archivo en disco está correcto). Protocolo confirmado: escribir el contenido vía heredoc a `/tmp`, verificar con `grep -c`/`wc -l`, y recién entonces copiar al destino. No regenerar a ciegas por lo que muestra el preview.
- **`node --check` como árbitro objetivo:** Ante duda de sintaxis (un paréntesis de más, etc.), `node --check archivo.js` decide. Valida sintaxis sin resolver imports, así que corre aún fuera del árbol de `node_modules`.
- **"Gana la evidencia":** Si un diagnóstico con datos reales refuta la hipótesis de partida, NO ejecutar el refactor pensado a priori. Ej.: el supuesto "bug sistémico de id_local" se descartó al verificar que todos los `id_local` estaban limpios (9 dígitos, sin nulls) — se evitó un refactor innecesario del core de persistencia, y la corrupción de "Carlos" resultó ser datos de prueba + falta de guards, no un bug de `id_local`. Y al revés: si la evidencia confirma una sospecha (ej. los 3 guards de idempotencia faltantes que sí existían), avanzar con confianza sin sobre-deliberar.
