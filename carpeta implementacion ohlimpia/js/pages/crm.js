...
// Cartuchos de color de responsable: misma funcion que usa Precios para Grupo y
// Resp. Neg., para que un responsable se vea igual en las dos pantallas.
const { chipEtq, fondoEtq, textoContraste } = await import(`../shared/etiquetas-color.js${q}`);
const { wireResizeColumnas, clampAncho } = await import(`../shared/columnas-resize.js${q}`);
const { acortarNombre } = await import(`../shared/nombres.js${q}`);
const { abrirPopupLista } = await import(`../shared/filtro-popup.js${q}`);
const { confirmar } = await import(`../shared/confirmar.js${q}`);
const { wireAltoTabla } = await import(`../shared/alto-tabla.js${q}`);
// Abre la nota de aumento guardada en Storage. La misma que se abre desde la columna
// Nota de Precios: un solo archivo, dos puertas.
